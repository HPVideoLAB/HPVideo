...
<div class="w-full mt-3 mb-4">
	<div class="flex w-full">
		<div class="flex justify-center flex-col items-center w-full {$mobile ? '' : (sizeflag ? 'max-w-[600px]' : 'max-w-[300px]')} rounded-lg
			...
			{#if isLastMessage}
				<button class="flex flex-row items-center bg-gray-50 rounded-lg px-4 py-1.5 text-gray-800 text-sm mt-2"
					on:click={ async () => { await resentMessageHandler() }}>{$i18n.t("Regenerate")}</button>
			{/if}
		</div>
	</div>
</div>