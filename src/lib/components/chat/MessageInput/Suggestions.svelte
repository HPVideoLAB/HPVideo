...
{#if prompts.length > 0}
  <div class="mb-2 flex gap-1 text-sm font-medium items-center text-gray-600 dark:text-gray-600">
    <iconify-icon icon="ph:lightbulb-fill" />
    {$i18n.t('Suggested')}
  </div>
{/if}
  ...
  >
    {#each prompts as prompt, promptIdx}
      <div class="snap-center shrink-0">
        <button
          class="flex flex-col flex-1 shrink-0 w-64 justify-between h-32 p-5 px-6 hover:bg-gradient-to-t from-[#9802E5] to-[#E0A3FF]
        ...
        >
          <div class="flex flex-col text-left">
            {#if prompt.title && prompt.title[0] !== ''}
              <div class="text-sm font-bold dark:text-gray-300 group-hover:text-white transition">
                {$i18n.t(prompt.title[0])}
              </div>
              <div class="text-xs text-gray-600 group-hover:text-white font-normal line-clamp-2">
          ...
          <div class="w-full flex justify-between">
            <div class="text-xs text-gray-400 group-hover:text-white dark:text-gray-600 transition self-center">
              {$i18n.t('Prompt')}
            </div>
