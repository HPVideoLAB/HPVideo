...
1. **永不亏钱**：每次 API 消耗 `credits charged >= 2 × API_cost_USD × 100`（50% 毛利兜底）。
...
**1 积分 = $0.01 零售价值**（用户视角：100 积分 = $1）

**消耗公式**：`credits_charged = ceil(api_cost_usd × 100) × 2`
...
|---|---:|---:|---:|
| **ovi 540p/5s** | $0.225 | **45 积分** | $0.45 |
| **seedance 6s** | $0.30 | **60 积分** | $0.60 |
| **seedance 9s** | $0.45 | **90 积分** | $0.90 |
| **hailuo 6s** | $0.345 | **70 积分** | $0.70 |
| **wan-2.7 480p/5s** | $0.375 | **75 积分** | $0.75 |
| **vidu-q3 4s** | $0.40 | **80 积分** | $0.80 |
| **ltx-2.3 6s** | $0.54 | **108 积分** | $1.08 |
| **pixverse 5s** | $0.60 | **120 积分** | $1.20 |
| **wan-2.7 720p/5s** | $0.75 | **150 积分** | $1.50 |
| **luma-ray-2 5s** | $0.75 | **150 积分** | $1.50 |
| **wan-2.7 1080p/5s** | $1.125 | **225 积分** | $2.25 |
| **pixverse 8s** | $1.20 | **240 积分** | $2.40 |
| **luma-ray-2 10s** | $1.50 | **300 积分** | $3.00 |
| **kling-3.0 5s** | $2.10 | **420 积分** | $4.20 |
| **wan-2.7 1080p/10s** | $2.25 | **450 积分** | $4.50 |
| **veo3.1 4s** | $2.40 | **480 积分** | $4.80 |
| **veo3.1 6s** | $3.60 | **720 积分** | $7.20 |
| **kling-3.0 10s** | $4.20 | **840 积分** | $8.40 |
| **veo3.1 8s** | $4.80 | **960 积分** | $9.60 |
...
| 套餐 | 价格 | 月度积分 | 性价比 | 关键特权 |
|---|---:|---:|---:|---|
| **Free** | $0 | 30/天（不可累积） | - | 仅低端 5 模型，480p，带水印，1 并发 |
| **Creator** | $9.99/月 | 1,200 积分 | $0.0083/积分 | 无水印，1080p，2 并发，所有模型 |
| **Pro** | $29/月 | 4,000 积分 | $0.00725/积分 | 4 并发，优先队列，30 天积分滚存，商用授权 |
| **Studio** | $99/月 | 15,000 积分 | $0.0066/积分 | 8 并发，4K（wan-2.7 1080p+），API Key，团队 3 席，链上版税收益 |

### 订阅毛利测算（按全消耗算最坏情况）

| 套餐 | 收入 | 用户最大消耗（按 credit 价值） | 实际 API 成本（消耗 / 2） | Stripe 费 | 净毛利 |
|---|---:|---:|---:|---:|---:|
| Creator $9.99 | $9.99 | $12（1200 × $0.01）| **$6** | $0.30 | **$3.69 (37%)** |
| Pro $29 | $29 | $40 | **$20** | $0.87 | **$8.13 (28%)** |
| Studio $99 | $99 | $150 | **$75** | $2.97 | **$21.03 (21%)** |
...
- Free：当日清零（防囤积）
- Creator：当月清零
- Pro：30 天滚存，最多累积到 8000
- Studio：60 天滚存，最多累积到 30000
...
| 包 | 价格 | 积分 | 加赠 | 等效单价 | 备注 |
|---|---:|---:|---:|---:|---|
| **Starter** | $4.99 | 500 | 0 | $0.00998 | 入门试水 |
| **Plus** | $19.99 | 2,200 | +200（10%）| $0.00909 | 性价比款 |
| **Power** | $49.99 | 6,000 | +1000（20%）| $0.00833 | 重度用户 |
| **Whale** | $99.99 | 13,000 | +3000（30%）| $0.00769 | 工作室 / 团队 |

**最低毛利测试**（$99 包消耗到底）:
- 收入 $99.99
- 13,000 积分 × $0.005（2× 毛利兜底后实际成本）= **$65 API 成本**
- Stripe ~$3
- **净毛利 ~$32 (32%)** ✓ 不亏

**注意点**：加赠的积分让单价持续下降，但永远 ≥ $0.0077（>2× 标准消耗成本上限），所以最坏情况仍有 23% 毛利。
...
| 机制 | 实现 |
|---|---|
| Free 上限 | 每日 30 积分，1 视频，仅 5 个低端模型，480p+ 水印 |
...
**Free 真实成本上限测算**：
- 假设最差情况：用户每天用满 30 积分（≈ 1 个 ovi 5s 生成 = $0.15 成本）
...
后台自动创建 Privy Wallet（用户无感）
       ↓
首次进入 /creator/ → 弹"Welcome 礼包：100 免费积分（注册赠送）"
       ↓
...
async def charge_for_generation(charge: CreditCharge) -> bool:
    credits_needed = math.ceil(charge.cost_usd * 100) * 2  # 2x markup
